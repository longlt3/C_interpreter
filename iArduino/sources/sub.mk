...
out/definition_c_.o: ./definition.c 
	@/bin/echo -e 'Building target #2: '$(COLOR_GRN)'$<'$(COLOR_WHE)
	@/bin/echo '--------------------------------------'
	"$(cBuild)" -MMD -MP -MF"$(@:%.o=%.d)" @"$(MakeResDirPath)/includes.args" $(CFLAGS) $(INCFLAGS) "$<" @"$(MakeSrcDirPath)/definition_c_.args" @"$(MakeSrcDirPath)/sub.args" @"$(MakeResDirPath)/common.args" $(LDFLAGS) $(LIBFLAGS) -o "$@"
	@/bin/echo '======================================'
	@/bin/echo -e ''$(COLOR_BLE)'Finished building: $@'$(COLOR_WHE)''
	@/bin/echo ' '
